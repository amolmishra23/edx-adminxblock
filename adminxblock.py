...
class AdminXBlock(XBlock):
    ...
    def perform(self, data, suffix=''):
        ...
        # Functionality is yet to be implemented
            ...
            res += 'Functionality yet to be implemented'
        ...
        # and in the end, attempt to the user's name on the final screen, whose certificate was generated .
            ...
            db = MySQLdb.connect("localhost", "root", "", "edxapp")
            cursor = db.cursor()
            sql = 'select c.name from auth_userprofile a,certificates_generatedcertificate c where a.name = c.name and a.mailing_address="' + email + '" and c.status="downloadable"';
            cursor.execute(sql)
            results = cursor.fetchall()
            names_list = []
            for row in results:
            ...
            if not names_list:
                ...
                res += 'Certificate generation for particular user was successful for '+names_list[0]
